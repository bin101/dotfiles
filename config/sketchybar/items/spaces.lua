...
-- Subscribe to workspace change events to update dynamically
...
space_listener:subscribe("space_windows_change", function(env)
  -- Only update windows for the affected workspace
  local workspace = env.FOCUSED_WORKSPACE
  if workspace and workspaces[workspace] then
    updateSpaceWindows(workspaces[workspace].space, workspace)
  else
    -- Workspace might be new or removed, do a full update
    updateWorkspaces()
  end
end)
